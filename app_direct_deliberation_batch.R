# STANDALONE DIRECT DELIBERATION APP
# Generated by splitting the dual-condition app to prevent state leakage.
...
DIRECT_STEPS <- list(
  list(id = 0,  label = "Setup",                           icon = "\\u2699"),
  list(id = 1,  label = "Generate Participants",          icon = "\\U0001F465"),
  list(id = 5,  label = "Direct Deliberation",            icon = "\\U0001F5E3"),
  list(id = 14, label = "Report Generation",              icon = "\\U0001F3DB"),
  list(id = 15, label = "Quality Assessment",             icon = "\\u2705")
)
...
coerce_json <- function(txt) {
  ...
  # First repair attempt: strip JS-style // line comments (LLMs often emit these
  # inside JSON despite instructions, even though JSON disallows them).
  no_comments <- gsub("//[^\n]*", "", json_txt, perl = TRUE)
  result_nc <- tryCatch(fromJSON(no_comments, simplifyVector = FALSE), error = function(e) NULL)
  if (!is.null(result_nc)) return(result_nc)
...
# --- Direct deliberation task prompt (non-staged, multi-agent free discussion) ---
# Extracted from the inline string in the direct-condition server block so that
# the task text lives alongside the other prompt functions and can be documented
# / cited in a methods section. Semantics unchanged.
direct_task_prompt <- function(round_id, total_rounds) {
  paste0(
    "You are in a direct multi-agent deliberation condition. ",
    "Engage with the running discussion, respond to others when relevant, ",
    "and help the group work toward a collective judgment on the policy question. ",
    "You may change your view if persuaded, but do so only when the discussion ",
    "gives good reasons. Round ", round_id, " of ", total_rounds, ".")
}

# --- Transparency disclosure block (Rountree & Gastil 2026, p. 9) ---
# R&G are explicit that those running simulations should disclose "what software
# was used, what datasets were used to train the model, what prompts were used
# to run the simulation, how many times a simulation has been run, and how the
# inputs into and outputs from simulations are being used to inform decision-
# making." This helper emits a short plain-text block to prepend to the
# appendix of any final report.
build_transparency_disclosure <- function(rv, input, condition_label = "structured_pipeline") {
  n_stages <- if (!is.null(rv$replog) && is.data.frame(rv$replog)) nrow(rv$replog) else 0L
  n_turns  <- if (!is.null(rv$turns)  && is.data.frame(rv$turns))  nrow(rv$turns)  else 0L
  main_model    <- input$model %||% "(not set)"
  auditor_model <- input$auditor_model %||% main_model
  auditor_note  <- if (identical(main_model, auditor_model)) {
    " (same as main model — cross-model auditing disabled)"
  } else {
    " (cross-model auditing enabled per Rountree & Gastil 2026, p. 8)"
  }
  paste0(
    "--- TRANSPARENCY DISCLOSURE ---\n",
    "This block is generated automatically in line with the transparency criteria in\n",
    "Rountree, J., & Gastil, J. (2026). The Case for Using Generative AI to Run\n",
    "Deliberation Simulations. Journal of Deliberative Democracy, 21(1), pp. 1-14.\n\n",
    "Run ID: ", scalar_text(rv$run_id %||% "(not set)"), "\n",
    "Run started: ", scalar_text(rv$run_started_at %||% "(not set)"), "\n",
    "Run ended: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z"), "\n",
    "Condition: ", condition_label, "\n",
    "Main generation model: ", main_model, "\n",
    "Auditor model (Steps 2/3/15): ", auditor_model, auditor_note, "\n",
    "Sampling temperature: ", as.numeric(input$temperature %||% 0.3), "\n",
    "Participants: ", as.integer(input$n_participants %||% NA), "\n",
    "Groups: ", as.integer(input$n_groups %||% NA), "\n",
    "Simulation stages executed: ", n_stages, "\n",
    "Total agent turns recorded: ", n_turns, "\n",
    "Software: Deliberation Simulator (Shiny R app, v2) built on the\n",
    "Rountree & Gastil (2026) Appendix prompt sequence with Taubenfeld et al. (2024)\n",
    "bias-mitigation controls (persona reinforcement, per-turn drift cap) and\n",
    "Behrendt et al. (2024) AQuA deliberative-quality scoring.\n\n",
    "Intended use: This output is a DELIBERATION-MAKING aid, not a decision-\n",
    "making artefact. Per Rountree & Gastil (2026), simulated deliberation should\n",
    "complement, not replace, human judgement. Any downstream policy use should\n",
    "involve real stakeholders and treat simulated perspectives as provisional.\n",
    "\n",
    "Known limitations of this run: training-data bias in the underlying LLM(s);\n",
    "possible sycophancy and hallucination; no retrieval-augmented generation;\n",
    "no hybrid human+simulated participants in this condition.\n",
    "--- END TRANSPARENCY DISCLOSURE ---\n\n")
}
...
build_aqua_scoring_prompt <- function(comment_text) {
  # Build a clean numbered list of definitions (separated from the JSON schema),
  # then a JSON template with NO inline comments. Some LLMs fail to produce
  # valid JSON when the schema contains "// ..." JS-style comments because
  # they (correctly) recognize that JSON disallows them, and may either echo
  # the comments verbatim (breaking parse) or rename keys to compensate.
  defs_block <- paste(
    mapply(function(idx, name, question) {
      paste0(idx, ". ", name, ": ", question)
    },
    seq_along(AQUA_DIMENSION_PROMPTS),
    names(AQUA_DIMENSION_PROMPTS),
    AQUA_DIMENSION_PROMPTS),
    collapse = "\n"
  )
  schema_block <- paste(
    sapply(names(AQUA_DIMENSION_PROMPTS), function(name) paste0('  "', name, '": 0')),
    collapse = ",\n"
  )
  paste0(
    "You are an expert coder of deliberative discourse quality.\n",
    "Score the following discussion comment on each of the 20 binary dimensions.\n",
    "For each dimension, return exactly 0 (absent) or 1 (present), based strictly on what is explicitly present in the text. Do not infer intent.\n\n",
    "DIMENSION DEFINITIONS:\n", defs_block, "\n\n",
    "COMMENT TO SCORE:\n", comment_text, "\n\n",
    "Return ONLY a single JSON object with EXACTLY the 20 keys shown below (use these key names verbatim, including spaces and capitalisation). ",
    "Each value must be the integer 0 or the integer 1 (no booleans, no strings, no comments, no extra keys).\n",
    "###JSON_START###\n",
    "{\n",
    schema_block, "\n",
    "}\n",
...
# Helper: normalize a key for tolerant matching (lowercase, strip spaces/underscores/punct)
.aqua_normalize_key <- function(s) {
  gsub("[^a-z0-9]", "", tolower(as.character(s %||% "")))
}

score_single_comment_aqua <- function(comment_text, api_key, model, temperature = 0.0) {
  # Returns a named numeric vector of 20 binary scores (0/1).
  # On failure, returns an NA-filled vector with attr "aqua_error" describing why.
  ...
  if (!nzchar(trimws(comment_text %||% ""))) {
    attr(na_result, "aqua_error") <- "empty comment"
    return(na_result)
  }
  ...
  raw <- tryCatch(
    call_openrouter(
      api_key, model,
      list(
        list(role = "system", content = "You are a precise binary coder of deliberative discourse. Return only JSON. Use exactly the key names provided. Each value is the integer 0 or the integer 1."),
        list(role = "user",   content = build_aqua_scoring_prompt(comment_text))
      ),
      max_tokens = 600,         # raised from 400 to fit the full 20-key JSON safely
      temperature = temperature,
      max_retries = 2
    ),
    error = function(e) { attr(na_result, "aqua_error") <<- paste0("API call failed: ", conditionMessage(e)); NULL }
  )
  if (is.null(raw)) return(na_result)
  
  parsed <- tryCatch(coerce_json(raw), error = function(e) {
    attr(na_result, "aqua_error") <<- paste0("JSON parse failed: ", conditionMessage(e),
                                             " | raw[1:200]: ", substr(as.character(raw), 1, 200))
    NULL
  })
  if (is.null(parsed) || !is.list(parsed)) return(na_result)
  
  # Build a normalized-name lookup from the parsed object so we can tolerate
  # snake_case, lowercased, or otherwise-reformatted keys that some LLMs return
  # despite instructions.
  parsed_keys <- names(parsed)
  norm_lookup <- setNames(parsed_keys, vapply(parsed_keys, .aqua_normalize_key, character(1)))
  
  scores <- vapply(dim_names, function(d) {
    v <- parsed[[d]]
    if (is.null(v)) {
      # Try normalized match
      key_norm <- .aqua_normalize_key(d)
      hit <- if (key_norm %in% names(norm_lookup)) norm_lookup[[key_norm]] else NULL
      if (!is.null(hit)) v <- parsed[[hit]]
    }
    if (is.null(v)) return(NA_real_)
    # Tolerate boolean / "0" / "1" / "yes"/"no" responses
    if (is.logical(v)) return(as.numeric(v[[1]]))
    s <- tolower(trimws(as.character(v[[1]])))
    if (s %in% c("yes", "true", "present", "1")) return(1)
    if (s %in% c("no", "false", "absent", "0"))  return(0)
    suppressWarnings(as.numeric(s))
  }, numeric(1))
  
  # Clamp to 0/1 (only finite values; leave NAs as NAs so the caller can detect partial parse)
  finite_idx <- is.finite(scores)
  scores[finite_idx] <- pmax(0, pmin(1, round(scores[finite_idx])))
  
  # If any dimension is still NA, attach diagnostic so the caller can log it
  if (any(!finite_idx)) {
    missing_dims <- dim_names[!finite_idx]
    attr(scores, "aqua_error") <- paste0(
      "Partial parse: missing/unparseable dimensions = ",
      paste(missing_dims, collapse = ", "),
      " | parsed keys = ", paste(parsed_keys, collapse = ", ")
    )
  }
  scores
}
...
# Fallback AQuA scorer used when API scoring is unavailable or a single
# LLM coding call fails. This is deliberately conservative and transparent:
# it approximates the 20 binary indicators from explicit textual cues so the
# AQuA tab still produces inspectable outputs for uploaded/batch datasets.
score_single_comment_aqua_heuristic <- function(comment_text) {
  dim_names <- names(AQUA_WEIGHTS)
  txt <- scalar_text(comment_text %||% "")
  low <- tolower(txt)
  out <- setNames(rep(0, length(dim_names)), dim_names)
  if (!nzchar(trimws(txt))) return(out)
  
  has_pat <- function(pattern) grepl(pattern, low, perl = TRUE)
  wc <- length(strsplit(trimws(gsub("\\s+", " ", txt)), "\\s+")[[1]])
  
  out["Relevance"] <- as.numeric(wc >= 3)
  out["Fact"] <- as.numeric(has_pat("\\b(data|evidence|research|study|studies|statistics|statistical|percent|%|according to|report|documented|because of|due to)\\b|\\d"))
  out["Opinion"] <- as.numeric(has_pat("\\b(i think|i believe|in my view|in my opinion|i feel|we should|should|must|need to|it is important|i prefer|i agree|i disagree)\\b"))
  out["Justification"] <- as.numeric(has_pat("\\b(because|since|therefore|so that|as a result|this means|for this reason|due to|if .* then|in order to)\\b"))
  out["Solution Proposals"] <- as.numeric(has_pat("\\b(we should|should|could|must|need to|recommend|proposal|propose|solution|policy|implement|introduce|create|support|fund|regulate)\\b"))
  out["Additional Knowledge"] <- as.numeric(out["Fact"] == 1 || has_pat("\\b(for example|for instance|context|background|in other countries|evidence shows|research shows)\\b"))
  out["Question"] <- as.numeric(grepl("\\?", txt) || has_pat("\\b(what do you think|could you|would you|how can|why should|is it possible)\\b"))
  out["Referencing Users"] <- as.numeric(has_pat("\\b(you|your|we|us|everyone|others|participants|group|people here|colleagues|as .* said)\\b"))
  out["Referencing Medium"] <- as.numeric(has_pat("\\b(platform|moderator|moderation|organiser|organizer|forum|survey|app|article|media|newspaper)\\b"))
  out["Referencing Contents"] <- as.numeric(has_pat("\\b(previous comment|earlier point|as mentioned|as said|your argument|that point|this argument|the proposal above|what .* said)\\b"))
  out["Referencing Personal"] <- as.numeric(has_pat("\\b(your experience|your background|as a parent|as a worker|as a farmer|as a student|personally|my family|my job|my work)\\b"))
  out["Referencing Format"] <- as.numeric(has_pat("\\b(tone|wording|language|spelling|style|format|caps|punctuation)\\b"))
  out["Polite Form of Address"] <- as.numeric(has_pat("\\b(thank you|thanks|dear|hello|hi |good morning|good evening|regards|best wishes|please)\\b"))
  out["Respect"] <- as.numeric(has_pat("\\b(i respect|respectfully|i appreciate|good point|important point|valid point|i understand|fair point|thank you|thanks)\\b"))
  out["Screaming"] <- as.numeric(grepl("[!?]{3,}", txt) || grepl("\\b[A-Z]{5,}\\b", txt))
  out["Vulgar"] <- as.numeric(has_pat("\\b(fuck|shit|bullshit|crap|damn|bastard)\\b"))
  out["Insults"] <- as.numeric(has_pat("\\b(idiot|stupid|moron|liar|ignorant|nonsense|ridiculous person|fool)\\b"))
  out["Sarcasm"] <- as.numeric(has_pat("\\b(yeah right|sure,|obviously|so-called|as if|what a joke)\\b"))
  out["Discrimination"] <- as.numeric(has_pat("\\b(racist|sexist|discriminat|stereotype|inferior|all migrants|all women|all men|those people)\\b"))
  out["Storytelling"] <- as.numeric(has_pat("\\b(i remember|when i|in my experience|my experience|personally|my family|my child|my children|my job|my story|once)\\b"))
  
  out <- pmax(0, pmin(1, round(as.numeric(out))))
  names(out) <- dim_names
  out
}

compute_aqua_llm <- function(turns_df, api_key = "", model = "", temperature = 0.0, use_llm = TRUE) {
  if (is.null(turns_df) || !is.data.frame(turns_df) || nrow(turns_df) == 0 || !"text" %in% names(turns_df)) return(NULL)
  
  df <- turns_df
  n <- nrow(df)
  dim_names <- names(AQUA_WEIGHTS)
  scores_mat <- matrix(NA_real_, nrow = n, ncol = length(dim_names), dimnames = list(NULL, dim_names))
  scoring_method <- rep("heuristic_fallback", n)
  per_comment_errors <- rep(NA_character_, n)  # diagnostic: per-comment LLM failure reasons
  can_use_llm <- isTRUE(use_llm) && nzchar(clean_api_key(api_key)) && nzchar(model %||% "")
  
  for (i in seq_len(n)) {
    txt <- scalar_text(df$text[i] %||% "")
    if (!nzchar(trimws(txt))) {
      scores_mat[i, ] <- setNames(rep(0, length(dim_names)), dim_names)
      scoring_method[i] <- "empty_comment"
      next
    }
    
    if (can_use_llm) {
      llm_scores <- tryCatch(
        score_single_comment_aqua(txt, api_key, model, temperature),
        error = function(e) {
          v <- setNames(rep(NA_real_, length(dim_names)), dim_names)
          attr(v, "aqua_error") <- paste0("uncaught error: ", conditionMessage(e))
          v
        }
      )
      err_attr <- attr(llm_scores, "aqua_error", exact = TRUE)
      if (!is.null(err_attr) && nzchar(err_attr)) per_comment_errors[i] <- err_attr
      
      finite_mask <- is.finite(llm_scores[dim_names])
      if (all(finite_mask)) {
        # All 20 dimensions parsed cleanly from the LLM.
        scores_mat[i, ] <- llm_scores[dim_names]
        scoring_method[i] <- "llm"
      } else if (any(finite_mask)) {
        # Partial parse: keep the LLM-valid dimensions, heuristic-fill the rest.
        # Marked as "llm_partial" so it doesn't count as fully LLM-scored in strict mode.
        heur <- score_single_comment_aqua_heuristic(txt)[dim_names]
        merged <- llm_scores[dim_names]
        merged[!finite_mask] <- heur[!finite_mask]
        scores_mat[i, ] <- merged
        scoring_method[i] <- "llm_partial"
      } else {
        # No dimension parsed; full heuristic fallback for this comment.
        scores_mat[i, ] <- score_single_comment_aqua_heuristic(txt)[dim_names]
        scoring_method[i] <- "heuristic_after_llm_failure"
      }
      if (i < n) Sys.sleep(0.3)
    } else {
      scores_mat[i, ] <- score_single_comment_aqua_heuristic(txt)[dim_names]
      scoring_method[i] <- "heuristic_fallback"
    }
  }
  
  ind <- as.data.frame(scores_mat)
  for (d in dim_names) ind[[d]] <- ifelse(is.na(ind[[d]]), 0, ind[[d]])
  
  raw   <- as.matrix(ind[, dim_names, drop = FALSE]) %*% AQUA_WEIGHTS
  ...
  comments <- cbind(df[, intersect(c("turn", "speaker", "phase", "group", "text"), names(df)), drop = FALSE], ind)
  comments$aqua_score <- score
  comments$aqua_scoring_method <- scoring_method
  comments$aqua_error <- per_comment_errors
  
  if (!"speaker" %in% names(comments)) comments$speaker <- "Unknown"
  if (!"phase" %in% names(comments)) comments$phase <- "Unknown"
  if (!"group" %in% names(comments)) comments$group <- "All"
  
  # Summarize the most common per-comment failure reason for diagnostics.
  failure_reasons <- per_comment_errors[!is.na(per_comment_errors) & nzchar(per_comment_errors)]
  failure_summary <- if (length(failure_reasons) == 0) NA_character_ else {
    tab <- sort(table(substr(failure_reasons, 1, 120)), decreasing = TRUE)
    paste0(names(tab)[1], " (", tab[[1]], "/", length(failure_reasons), " failures)")
  }
  ...
  overview <- data.frame(
    ...
    n_llm_scored = sum(scoring_method == "llm", na.rm = TRUE),
    n_fallback_scored = sum(scoring_method != "llm", na.rm = TRUE),
    stringsAsFactors = FALSE
  )
  ...
  list(
    ...
    method_note = if (can_use_llm) {
      "AQuA was scored using the selected LLM; failed comment-level calls used the transparent heuristic fallback."
    } else {
      "AQuA was scored using the transparent heuristic fallback because no API/model was available."
    }
  )
}
...
ui <- fluidPage(
  ...
  tags$div(id = "main-content",
           tags$div(class = "api-bar",
                    ...
                    tags$div(style = "flex:1;min-width:240px;", tags$label("Auditor Model (Integrity + Quality)"), uiOutput("auditor_model_ui")),
           ...
           tags$div(class = "small-note", style = "margin:-14px 0 16px 0; padding:0 24px;",
                    ...
                    "The Auditor Model is used for the Design Integrity review (Step 3) and the Quality Assessment (Step 15). Selecting a different provider from the main model implements Rountree & Gastil's (2026, p. 8) recommendation of cross-model scrutiny to mitigate single-model sycophancy. Leave equal to the main model to disable cross-model auditing."),
           uiOutput("condition_tabs_ui"),
...
server <- function(input, output, session) {
  ...
  rv <- reactiveValues(
    ...
    # Run-level provenance for the transparency disclosure block
    # (Rountree & Gastil 2026, p. 9: what software was used, what prompts,
    # how many runs, how outputs inform decision-making).
    run_id = paste0("run-", format(Sys.time(), "%Y%m%d-%H%M%S"), "-",
    ...
    personas = NULL, base_personas = NULL, integrity = NULL, agent_cards = NULL, groups = NULL,
    group_values = NULL, master_values = NULL, group_options = NULL, master_options = NULL,
    direct_deliberation = NULL,
    ...
    current_condition = "direct_agents",
    condition_snapshots = list(pipeline = NULL, direct_agents = NULL),
  ...
  # Direct Deliberation must use the personas generated inside the Direct tab only.
  # It must NOT inherit the population from the structured pipeline, because that would
  # contaminate the comparison and make the two conditions non-independent.
  baseline_personas_for_direct <- function() {
    snap <- rv$condition_snapshots$direct_agents
    p <- NULL
    
    # If the user is currently on the Direct tab, use the current Direct-tab state.
    if (identical(rv$current_condition %||% "pipeline", "direct_agents")) {
      p <- rv$base_personas %||% rv$personas
    }
    
    # Otherwise use the saved Direct-tab state, if it exists.
    if (is.null(p) && !is.null(snap)) {
      p <- snap$base_personas %||% snap$personas
    }
    
    if (is.null(p)) return(NULL)
    p <- as.data.frame(p, stringsAsFactors = FALSE)
    outcome_cols <- intersect(names(p), c("final_score", "score_after", "post_score"))
    if (length(outcome_cols)) p[outcome_cols] <- NULL
    p
  }
  ...
  condition_fields <- c(
    "view", "step", "personas", "base_personas", "integrity", "agent_cards", "groups",
    "group_values", "master_values", "group_options", "master_options",
    "evaluations", "final_report", "appendix_report", "quality", "transcript",
    "turns", "rankings", "persona_validated", "model_baseline", "drift_table",
    "pre_survey", "post_survey", "auto_dri_survey", "user_dri_survey",
    "auto_dri_pre", "auto_dri_post", "user_dri_pre", "user_dri_post",
    "auto_dri_pre_scores", "auto_dri_post_scores", "user_dri_pre_scores", "user_dri_post_scores",
    "auto_dri_comparison", "user_dri_comparison", "central_tendency", "aqua",
    "direct_deliberation", "direct_final_report", "direct_appendix_report", "direct_transcript"
  )
  
  save_condition_snapshot <- function(cond = rv$current_condition) {
    snap <- list()
    for (nm in condition_fields) snap[[nm]] <- if (!is.null(rv[[nm]])) rv[[nm]] else NULL
    rv$condition_snapshots[[cond]] <- snap
  }
  
  reset_condition_state <- function(cond = rv$current_condition) {
    rv$view <- "step"
    rv$step <- 0L
    for (nm in setdiff(condition_fields, c("view", "step"))) rv[[nm]] <- NULL
    rv$persona_validated <- FALSE
  }
  
  load_condition_snapshot <- function(cond = rv$current_condition) {
    snap <- rv$condition_snapshots[[cond]]
    if (is.null(snap)) {
      reset_condition_state(cond)
    } else {
      for (nm in condition_fields) rv[[nm]] <- snap[[nm]]
      if (is.null(rv$view)) rv$view <- "step"
      if (is.null(rv$step)) rv$step <- 0L
      if (is.null(rv$persona_validated)) rv$persona_validated <- FALSE
    }
  }
  
  sync_shared_setup_to_both_conditions <- function(set_direct_step = TRUE) {
    # EXPERIMENTAL INDEPENDENCE RULE
    # The two conditions share ONLY measurement instruments and high-level setup.
    # They do NOT share mutable experimental material such as personas, validation edits,
    # group allocations, transcripts, reports, DRI responses, DRI scores, quality outputs,
    # AQuA, or central tendency outputs.
    #
    # Consequence: the Direct Deliberation population must be generated in the Direct tab
    # using the manual/direct settings. The structured-pipeline population is not copied
    # into the direct condition.
    if (is.null(rv$condition_snapshots$pipeline)) rv$condition_snapshots$pipeline <- list()
    if (is.null(rv$condition_snapshots$direct_agents)) rv$condition_snapshots$direct_agents <- list()
    
    seed_condition <- function(cond, next_step) {
      snap <- rv$condition_snapshots[[cond]]
      if (is.null(snap)) snap <- list()
      
      # Measurement instruments are copied to both conditions so the comparison uses
      # the same DRI questionnaire. Responses/scores remain condition-specific.
      if (!is.null(rv$auto_dri_survey)) snap$auto_dri_survey <- rv$auto_dri_survey
      if (!is.null(rv$user_dri_survey)) snap$user_dri_survey <- rv$user_dri_survey
      if (!is.null(rv$model_baseline)) snap$model_baseline <- rv$model_baseline
      
      if (is.null(snap$view)) snap$view <- "step"
      if (is.null(snap$step) || isTRUE(snap$step < 1L)) snap$step <- next_step
      snap
    }
    
    rv$condition_snapshots$pipeline <- seed_condition("pipeline", 1L)
    rv$condition_snapshots$direct_agents <- seed_condition("direct_agents", 1L)
  }
  
  active_condition <- reactive({ "direct_agents" })
  
  output$model_ui <- renderUI({ selectInput("model", label = NULL, choices = rv$models, selected = rv$models[1]) })
  output$auditor_model_ui <- renderUI({ selectInput("auditor_model", label = NULL, choices = rv$models, selected = rv$models[1]) })
  
  output$condition_tabs_ui <- renderUI({
    tags$div(
      style = "padding:0 24px 8px 24px;",
      tags$div(class = "info-box",
               tags$b("Direct Deliberation only"),
               tags$p("This standalone app has its own state. It does not load, inherit, validate, or modify personas from the structured-pipeline app.")
      )
    )
  })
  
  observe({ rv$current_condition <- "direct_agents" })
  observeEvent(input$refresh_models, {
    tryCatch({
      ...
      updateSelectInput(session, "auditor_model", choices = mods, selected = mods[1])
      ...
      updateSelectInput(session, "auditor_model", choices = DEFAULT_MODELS, selected = DEFAULT_MODELS[1])
  ...
  output$sidebar_steps <- renderUI({
    current_steps <- if (identical(active_condition(), "direct_agents")) DIRECT_STEPS else STEPS
    tagList(lapply(current_steps, function(s) {
  ...
  output$main_ui <- renderUI({
    ...
    if (rv$view == "condition_compare") return(build_condition_compare_ui())
    build_step_ui()
  })
  ...
  build_step_ui <- function() {
    s <- rv$step
    experiment_mode <- active_condition()
    if (s == 0) {
      return(tagList(
        ...
        tags$div(
          class = "demo-section",
          tags$h4("Experiment Condition"),
          tags$p(class = "small-note", if (identical(experiment_mode, "direct_agents")) "You are currently on the Direct Deliberation tab. This condition keeps the same setup and metrics, but skips the staged values-options-evaluation pipeline and lets all personas deliberate directly in one shared exchange." else "You are currently on the Structured Pipeline tab. This condition runs the original normatively driven pipeline with values, option generation, evaluation, final recommendation, and the standard output suite."),
          if (identical(experiment_mode, "direct_agents")) tagList(
            numericInput("direct_rounds", "Direct deliberation rounds", value = 2, min = 1, max = 3, step = 1),
            numericInput("direct_max_turns", "Maximum direct discussion turns", value = 30, min = 5, max = 60, step = 5),
            tags$p(class = "small-note", "Cost guard: the direct condition stops after the selected number of rounds or the maximum turn cap, whichever comes first. This condition uses the participants generated in the Direct tab only; it does not inherit personas or validation edits from the structured pipeline.")
          )
        ),
        ...
        tags$div(
          ...
          tags$p(class = "small-note", "Upload a survey as PDF. Each agent will complete it before and after deliberation, enabling pre/post comparison on the Dashboard."),
          fileInput("survey_file", "Upload survey (PDF)", accept = ".pdf"),
          tags$hr(),
          tags$h4("🧠 Optional: Independent DRI Survey"),
          tags$p(class = "small-note", "Upload an external DRI survey in CSV. If provided, the app will show both automated DRI results and uploaded-survey DRI results."),
          tags$p(class = "small-note", "CSV must contain columns: section, id, text. Rows with 'consideration' will be rated on a 1–7 Likert scale. Rows with 'policy_option' will be ranked."),
          tags$a(href = "DRI_survey_template.csv", "Download CSV template", target = "_blank"),
          fileInput("dri_survey_file", "Upload independent DRI survey (CSV)", accept = c(".csv"))
        ),
        
        tags$hr(),
        textAreaInput("persona_note", "Additional persona guidance (optional)", rows = 3, placeholder = "e.g., include rural parents, teenagers, school counsellors..."),
        textInput("info_type", "Type of Background Information", placeholder = "e.g., evidence summary, expert testimony"),
        textAreaInput("info_base", "Typed Background Information", rows = 8, placeholder = "Paste the evidence or context here..."),
        fileInput("evidence_files", "Upload supporting documents / evidence", multiple = TRUE, accept = c(".txt", ".md", ".csv", ".tsv", ".json", ".pdf")),
        tags$div(class = "small-note", "Replicability is improved through explicit model selection, stored temperature, participant/group counts, and a run log."),
        tags$button(class = "btn-start", onclick = "Shiny.setInputValue('begin_click', Math.random(), {priority:'event'})", "Begin Deliberation \u2192"),
        tags$hr(),
        tags$div(
          class = "survey-section",
          tags$h4("Batch runs / concrete replications"),
          tags$p(class = "small-note", "Run the full direct-deliberation app multiple times with the current settings. The app stores two cumulative CSV datasets: individual-level rows and group-level rows, with run_id identifying each replication."),
          numericInput("batch_n_runs", "Number of concrete runs", value = 1, min = 1, max = 50, step = 1),
          checkboxInput("batch_aqua_strict",
                        "Strict AQuA mode: if any comment can't be LLM-scored, set the run's AQuA columns to NA (aqua_method_summary = 'failed_strict_llm'). The run itself is always saved \u2014 deliberation data is never discarded. Filter by aqua_method_summary == 'pure_llm' for clean AQuA analysis.",
                        value = TRUE),
    ...
    if (identical(experiment_mode, "direct_agents")) {
      step_cfgs <- list(
        "1" = list(t = "Generate Participants", d = "Generate stakeholder personas matching your demographic targets. This step also prepares the DRI instruments and all pre-deliberation survey baselines for the direct condition.", btn = "Generate Participants"),
        "5" = list(t = "Direct Deliberation", d = "All personas deliberate directly in one shared discussion, without persona validation or other intermediate pipeline stages. The same issue, evidence, DRI, and export metrics are retained.", btn = "Run Direct Deliberation"),
        "14" = list(t = "Report Generation", d = "Synthesize a final report and appendix from the direct deliberation transcript, then run the post-deliberation surveys and DRI.", btn = "Generate Report"),
        "15" = list(t = "Quality Assessment", d = "Assess the quality of the direct-agent deliberation with the same indicators used for the structured pipeline.", btn = "Run Quality Assessment")
      )
    } else {
      step_cfgs <- list(
    ...
    if (s == 1 && !is.null(rv$personas)) cb <- tagList(tags$div(class = "info-box", paste0(nrow(rv$personas), " personas generated.")), DTOutput("personas_table"))
    ...
    else if (s == 5 && identical(experiment_mode, "direct_agents") && !is.null(rv$direct_deliberation)) cb <- tags$div(class = "report-box", paste0(rv$direct_deliberation$summary, "\n\n---\n\nTRANSCRIPT PREVIEW\n\n", rv$direct_deliberation$transcript_preview))
      ...
      cb <- tagList(tags$div(class = "plot-wrap", plotOutput("plot_quality_step", height = "360px")), tagList(lapply(seq_along(QUALITY_DIMS), function(i) { d <- QUALITY_DIMS[i]; q <- rv$quality$quality[[d]]; sc <- if (is.null(q$score)) "?" else scalar_text(q$score); nt <- if (is.null(q$notes)) "" else scalar_text(q$notes); tags$div(class = "qa-card", tags$h5(QUALITY_LABELS[i]), tags$span(class = "qa-score", sc), tags$div(class = "qa-notes", nt)) })), if (!is.null(rv$quality$quality$recommendations)) tags$div(class = "qa-card", tags$h5("Recommendations"), tags$div(class = "qa-notes", paste("•", vapply(rv$quality$quality$recommendations, scalar_text, character(1)), collapse = "\n"))))
    }
    
    done <- if (identical(experiment_mode, "direct_agents")) {
      switch(as.character(s), "1" = !is.null(rv$personas), "5" = !is.null(rv$direct_deliberation), "14" = !is.null(rv$final_report), "15" = !is.null(rv$quality), FALSE)
    } else {
      switch(as.character(s), "1" = !is.null(rv$personas), "2" = rv$persona_validated, "3" = !is.null(rv$integrity), "4" = !is.null(rv$agent_cards), "5" = !is.null(rv$groups) && is.null(rv$group_values), "6" = !is.null(rv$group_values), "7" = !is.null(rv$master_values), "8" = !is.null(rv$group_options), "9" = !is.null(rv$group_options), "10" = !is.null(rv$master_options), "11" = !is.null(rv$evaluations), "12" = !is.null(rv$evaluations), "13" = !is.null(rv$rankings), "14" = !is.null(rv$final_report), "15" = !is.null(rv$quality), FALSE)
    }
    
    next_step <- if (identical(experiment_mode, "direct_agents")) switch(as.character(s), "1" = 5L, "5" = 14L, "14" = 15L, NA_integer_) else if (s < 15) s + 1L else NA_integer_
    
    btn <- if (isTRUE(rv$loading)) {
      tags$button(class = "btn-run", disabled = "disabled", "Running... please wait")
    } else if (!done) {
      tags$button(class = "btn-run", onclick = paste0("Shiny.setInputValue('run_current',", s, ",{priority:'event'})"), cfg$btn)
    } else if (!is.na(next_step)) {
      tags$button(class = "btn-next", onclick = paste0("Shiny.setInputValue('goto_step',", next_step, ",{priority:'event'})"), "Next Step \u2192")
    } else {
      tags$button(class = "btn-output", onclick = "Shiny.setInputValue('goto_output','report',{priority:'event'})", "View Outputs \u2192")
    }
    
    tagList(tags$div(class = "step-num", paste("Step", s)), tags$h1(class = "step-title", cfg$t), tags$p(class = "step-desc", cfg$d), cb, btn)
  }
  
  build_condition_compare_ui <- function() {
    pipe <- rv$condition_snapshots$pipeline
    direct <- rv$condition_snapshots$direct_agents
    metric_box <- function(title, snap) {
      tags$div(
        class = "card",
        tags$h3(title),
        if (is.null(snap)) tags$div(class = "info-box", "No run saved yet for this condition.") else tagList(
          tags$div(class = "kpi-grid",
                   tags$div(class = "kpi", tags$div(class = "kpi-value", if (!is.null(snap$final_report)) "Yes" else "No"), tags$div(class = "kpi-label", "final report")),
                   tags$div(class = "kpi", tags$div(class = "kpi-value", if (!is.null(snap$turns) && is.data.frame(snap$turns)) nrow(snap$turns) else 0), tags$div(class = "kpi-label", "turns")),
                   tags$div(class = "kpi", tags$div(class = "kpi-value", if (!is.null(snap$auto_dri_pre_scores)) round(snap$auto_dri_pre_scores$group_dri, 2) else "NA"), tags$div(class = "kpi-label", "auto DRI pre")),
                   tags$div(class = "kpi", tags$div(class = "kpi-value", if (!is.null(snap$auto_dri_post_scores)) round(snap$auto_dri_post_scores$group_dri, 2) else "NA"), tags$div(class = "kpi-label", "auto DRI post")),
                   tags$div(class = "kpi", tags$div(class = "kpi-value", if (!is.null(snap$user_dri_pre_scores)) round(snap$user_dri_pre_scores$group_dri, 2) else "NA"), tags$div(class = "kpi-label", "user DRI pre")),
                   tags$div(class = "kpi", tags$div(class = "kpi-value", if (!is.null(snap$user_dri_post_scores)) round(snap$user_dri_post_scores$group_dri, 2) else "NA"), tags$div(class = "kpi-label", "user DRI post"))
          ),
          tags$h4("Final report"),
          tags$div(class = "report-box", scalar_text((snap$final_report %||% snap$direct_final_report %||% "No final report generated yet."))),
          if (!is.null(snap$transcript) || !is.null(snap$direct_transcript)) tagList(
            tags$h4("Transcript"),
            tags$div(class = "report-box", scalar_text(snap$transcript %||% snap$direct_transcript))
          )
        )
      )
    }
    tagList(
      tags$div(class = "step-num", "Condition Comparison"),
      tags$h1(class = "step-title", "Structured pipeline vs direct deliberation"),
      tags$p(class = "step-desc", "Run each condition from its own tab. This page keeps the latest saved outputs for both conditions side by side for inspection and comparison."),
      fluidRow(
        column(6, metric_box("Structured Pipeline", pipe)),
        column(6, metric_box("Direct Deliberation", direct))
      )
    )
  }
  ...
  build_aqua_analysis_ui <- function() {
    ...
    tagList(
      ...
      tags$p(class = "step-desc", "AQuA scores are computed from the deliberation transcript/turn table. Use the button below if AQuA has not appeared automatically after a run. The app first tries LLM-based binary coding when an API key/model are available, and otherwise uses a transparent heuristic fallback so the AQuA tables and plots are always generated from available turns."),
      tags$div(class = "dl-row",
               actionButton("run_aqua_now", "Compute / refresh AQuA now", class = "btn-output"),
               tags$span(class = "small-note", "Works after any direct or structured deliberation that produced participant turns.")),
      ...
      if (!is.null(rv$aqua) && !is.null(rv$aqua$method_note)) tags$div(class = "info-box", rv$aqua$method_note),
  ...
  output$table_aqua_comments <- renderDT({ req(rv$aqua); datatable(rv$aqua$comments[, intersect(c("turn","speaker","phase","group","aqua_score","aqua_scoring_method","text"), names(rv$aqua$comments)), drop = FALSE], options = list(pageLength = 15, scrollX = TRUE), rownames = FALSE) })
  
  compute_aqua_from_current_turns <- function(prefer_llm = TRUE) {
    turns_for_aqua <- tryCatch(get_turns(), error = function(e) NULL)
    if (is.null(turns_for_aqua) || !is.data.frame(turns_for_aqua) || nrow(turns_for_aqua) == 0 || !"text" %in% names(turns_for_aqua)) {
      rv$status <- "AQuA could not run because no deliberation turns are available yet."
      return(invisible(NULL))
    }
    rv$aqua <- tryCatch(
      compute_aqua_llm(turns_for_aqua, input$api_key %||% "", input$model %||% "", temperature = 0.0, use_llm = prefer_llm),
      error = function(e) {
        rv$status <- paste0("AQuA LLM scoring failed; using heuristic fallback. Details: ", conditionMessage(e))
        compute_aqua_llm(turns_for_aqua, api_key = "", model = "", temperature = 0.0, use_llm = FALSE)
      }
    )
    if (!is.null(rv$aqua)) {
      rv$status <- paste0("AQuA computed for ", rv$aqua$overview$n_comments[1], " comments.")
      add_replog("aqua_scoring_manual_or_auto")
    } else {
      rv$status <- "AQuA could not be computed from the current turns."
    }
    invisible(rv$aqua)
  }
  
  observeEvent(input$run_aqua_now, {
    withProgress(message = "Computing AQuA from current deliberation turns...", value = 0.2, {
      compute_aqua_from_current_turns(prefer_llm = TRUE)
  ...
  observeEvent(input$dri_survey_file, {
    ...
    tryCatch({
      ...
      if (is.null(rv$condition_snapshots$pipeline)) rv$condition_snapshots$pipeline <- list()
      if (is.null(rv$condition_snapshots$direct_agents)) rv$condition_snapshots$direct_agents <- list()
      rv$condition_snapshots$pipeline$user_dri_survey <- rv$user_dri_survey
      rv$condition_snapshots$direct_agents$user_dri_survey <- rv$user_dri_survey
      rv$error <- ""
    }, error = function(e) {
      rv$user_dri_survey <- NULL
      rv$error <- conditionMessage(e)
  ...
  # Prepare DRI instruments and pre-deliberation responses for the active condition.
  # This is needed especially for Direct Deliberation, which skips Step 2 persona validation
  # but still must have the same pre/post DRI logic as the structured pipeline.
  ensure_dri_pre_responses <- function(personas_df = NULL, force_regenerate_survey = FALSE) {
    p <- personas_df %||% rv$base_personas %||% rv$personas
    if (is.null(p) || nrow(p) == 0) return(invisible(FALSE))
    p <- as.data.frame(p, stringsAsFactors = FALSE)
    cards <- lapply(seq_len(nrow(p)), function(i) as.list(p[i, ]))
    issue_local <- input$policy_issue %||% ""
    question_local <- input$policy_question %||% ""
    evidence_local <- build_evidence_block(input$info_base %||% "", rv$evidence_text, rv$evidence_names)
    temp_local <- min(1.0, max(0, as.numeric(input$temperature %||% 0.3)))
    
    if (force_regenerate_survey || is.null(rv$auto_dri_survey)) {
      auto_dri_txt <- call_openrouter(
        ...
        list(
          ...
          list(role = "user", content = dri_survey_prompt(issue_local, question_local, evidence_local))
        ),
        max_tokens = 3500,
        temperature = 0.2
      )
    ...
    if (is.null(rv$auto_dri_pre) && !is.null(rv$auto_dri_survey)) {
      auto_pre_results <- list()
      for (i in seq_along(cards)) {
        auto_pre_results[[i]] <- tryCatch({
          complete_dri_response(input$api_key, input$model, cards[[i]], rv$auto_dri_survey, "pre", temperature = temp_local)
        }, error = function(e) {
    ...
    if (!is.null(rv$user_dri_survey) && is.null(rv$user_dri_pre)) {
      user_pre_results <- list()
      for (i in seq_along(cards)) {
        user_pre_results[[i]] <- tryCatch({
          complete_dri_response(input$api_key, input$model, cards[[i]], rv$user_dri_survey, "pre", temperature = temp_local)
        }, error = function(e) {
          stop(conditionMessage(e))
        })
      }
  ...
  run_workflow_step <- function(step_id_override = NULL) {
    req(!rv$loading); rv$loading <- TRUE; rv$error <- ""; rv$status <- "Starting..."
    run_notice_id <- showNotification("Running step... please do not click again.", duration = NULL, closeButton = FALSE, type = "message")
    on.exit({ rv$loading <- FALSE; try(removeNotification(run_notice_id), silent = TRUE) }, add = TRUE)
    ...
    tryCatch({
      ...
      # Safety guard: repeated/queued clicks must not restart an already completed
      # direct deliberation and spend tokens again. To rerun, regenerate/reset the
      # direct condition first.
      if (identical(experiment_mode, "direct_agents") && step_id == 5 && !is.null(rv$direct_deliberation)) {
        rv$step <- 14
        rv$status <- "Direct deliberation already completed. Existing transcript retained; go to Report Generation."
        save_condition_snapshot("direct_agents")
        rv$error <- ""
        return(invisible(NULL))
      }
      ...
      if (step_id == 1) {
        ...
        if (identical(experiment_mode, "direct_agents")) {
          withProgress(message = "Preparing direct-condition DRI pre-surveys...", value = 0.1, {
            ensure_dri_pre_responses(rv$base_personas %||% rv$personas)
            incProgress(0.9)
          })
        }
        rv$step <- if (identical(experiment_mode, "direct_agents")) 5 else 2
        rv$status <- paste0("Generated ", nrow(rv$personas), " participants for the active condition only. DRI instruments are shared, but populations remain independent across Structured Pipeline and Direct Deliberation.")
        save_condition_snapshot(experiment_mode)
        sync_shared_setup_to_both_conditions(set_direct_step = TRUE)
        add_replog("generate_personas")
      }
      ...
      if (step_id == 2) {
        withProgress(message = "Validating personas...", value = 0.1, {
          ...
          # Use auditor model for cross-model scrutiny (Rountree & Gastil 2026, p. 8)
          auditor <- input$auditor_model %||% input$model
          out_int <- call_openrouter(
            input$api_key, auditor,
            list(
      ...
      if (step_id == 3) {
        withProgress(message = "Design integrity...", value = 0.3, {
          ...
          # Use auditor model for cross-model scrutiny (Rountree & Gastil 2026, p. 8)
          auditor <- input$auditor_model %||% input$model
          out <- call_openrouter(input$api_key, auditor,
      ...
      if (step_id == 5 && identical(experiment_mode, "direct_agents")) {
        if (isTRUE(rv$direct_lock)) {
          rv$status <- "Direct deliberation is already running. Please wait for the current run to finish."
          return(invisible(NULL))
        }
        rv$direct_lock <- TRUE
        on.exit({ rv$direct_lock <- FALSE }, add = TRUE)
        direct_personas <- baseline_personas_for_direct()
        if (is.null(direct_personas) || nrow(as.data.frame(direct_personas)) == 0) {
          rv$error <- "Direct Deliberation has no independent participant population yet. Open the Direct Deliberation tab and run Generate Participants there first. The direct condition no longer inherits personas from the structured pipeline."
          rv$status <- "Direct condition not started."
          return(invisible(NULL))
        }
        # Direct condition always starts from its own clean copy of the Direct-tab personas.
        # Do not use rv$personas as modified by the structured pipeline.
        rv$personas <- direct_personas
        rv$agent_cards <- lapply(seq_len(nrow(direct_personas)), function(i) as.list(direct_personas[i, ]))
        rv$direct_deliberation <- NULL
        ...
        rv$transcript <- NULL
        rv$turns <- NULL
        rv$auto_dri_post <- NULL
        rv$auto_dri_post_scores <- NULL
        rv$user_dri_post <- NULL
        rv$user_dri_post_scores <- NULL
        rv$auto_dri_comparison <- NULL
        rv$user_dri_comparison <- NULL
        rv$central_tendency <- NULL
        req(rv$agent_cards)
        # Ensure the direct condition has its own DRI pre responses before the direct discussion starts.
        # This protects the direct branch when it is run before OR after the structured pipeline.
        ensure_dri_pre_responses(rv$personas)
        withProgress(message = "Direct agent deliberation...", value = 0, {
          ...
          all_turns <- get_turns()
          turn_counter <- if (nrow(all_turns) > 0) max(all_turns$turn) + 1 else 1
          shared_context <- ""
          transcript_lines <- c()
          direct_total_rounds <- max(1L, min(3L, as.integer(input$direct_rounds %||% 2L)))
          direct_max_turns <- max(5L, min(60L, as.integer(input$direct_max_turns %||% 30L)))
          total_calls <- max(1L, min(length(rv$agent_cards) * direct_total_rounds, direct_max_turns))
          turns_executed <- 0L
          
          for (round_id in seq_len(direct_total_rounds)) {
            for (idx in seq_along(rv$agent_cards)) {
              if (turns_executed >= direct_max_turns) break
              turns_executed <- turns_executed + 1L
              incProgress(1 / total_calls, detail = paste0("Direct turn ", turns_executed, "/", total_calls, " — round ", round_id, ", agent ", idx, "/", length(rv$agent_cards)))
              ...
              task_text <- direct_task_prompt(round_id, direct_total_rounds)
              turn <- tryCatch({
                txt <- call_openrouter(input$api_key, input$model,
                                       list(list(role = "system", content = agent_system_prompt(persona)),
                                            list(role = "user", content = agent_turn_prompt(persona, issue, question, evidence_block, shared_context, task_text, current_score = score_before, current_position_text = current_positions[[speaker_name]] %||% persona$initial_position_text))),
                                       max_tokens = 500, temperature = temp)
                coerce_json(txt)
              }, error = function(e) {
                list(text = paste0("[Turn failed: ", conditionMessage(e), "]"), position_score = score_before, sentiment_score = 0, shift_reason = "Turn failed; prior position retained.")
              })
              ...
              if (is.finite(score_before) && is.finite(score_after) && abs(score_after - score_before) > 1) score_after <- score_before + sign(score_after - score_before)
              current_scores[[speaker_name]] <- score_after
              if (nzchar(trimws(turn_text))) current_positions[[speaker_name]] <- turn_text
              line <- paste0("Round ", round_id, " — ", speaker_name, ": ", turn_text, " [score ", score_after, "/5]")
              transcript_lines <- c(transcript_lines, line)
              shared_context <- paste(c(shared_context, line), collapse = "\n")
              all_turns <- bind_rows(all_turns, data.frame(turn = turn_counter, speaker = speaker_name, sentiment_score = as.numeric(turn$sentiment_score %||% 0), position_score = score_after, score_before_turn = suppressWarnings(as.numeric(score_before)), shift_reason = scalar_text(turn$shift_reason %||% ""), phase = "Direct Deliberation", group = "Direct Condition", text = turn_text, word_count = length(strsplit(trimws(turn_text), "\\s+")[[1]]), stringsAsFactors = FALSE))
              turn_counter <- turn_counter + 1
            }
            if (turns_executed >= direct_max_turns) break
          }
          if (turns_executed >= direct_max_turns && length(rv$agent_cards) * direct_total_rounds > direct_max_turns) {
            transcript_lines <- c(transcript_lines, paste0("[SYSTEM NOTE: Direct deliberation stopped at the configured safety cap of ", direct_max_turns, " turns to control token/cost exposure.]"))
          }
          direct_summary <- call_openrouter(input$api_key, input$model,
                                            list(list(role = "system", content = "You synthesize direct deliberation transcripts."),
                                                 list(role = "user", content = synthesis_prompt("Summarize the direct deliberation", issue, question, evidence_block, paste(transcript_lines, collapse = "\n"), "Write a concise analytical summary of the direct deliberation. Identify the main positions, the strongest reasons offered, areas of convergence, unresolved disagreements, and the apparent collective direction."))),
                                            max_tokens = 1400, temperature = 0.25)
          rv$turns <- all_turns
          # Populate AQuA immediately after direct deliberation so the AQuA tab is not empty.
          # This automatic pass uses the fast heuristic fallback; the AQuA tab button can refresh using LLM scoring.
          rv$aqua <- tryCatch(compute_aqua_llm(rv$turns, api_key = "", model = "", temperature = 0.0, use_llm = FALSE), error = function(e) NULL)
          ...
          rv$direct_deliberation <- list(summary = direct_summary, transcript_preview = paste(utils::head(transcript_lines, 25), collapse = "\n"))
          rv$transcript <- paste(transcript_lines, collapse = "\n")
        })
        if (isTRUE(rv$batch_running)) {
          rv$view <- "step"; rv$step <- 0L
          rv$status <- "Batch direct deliberation completed; staying on setup/export screen."
        } else {
          rv$step <- 14; rv$status <- "Direct deliberation completed."
        }
        add_replog("direct_deliberation")
      }
      ...
      if (step_id == 6) {
        ...
        withProgress(message = "Values round...", value = 0, {
          ...
          rv$group_values <- res$groups; rv$turns <- res$turns; rv$aqua <- tryCatch(compute_aqua_llm(rv$turns, api_key = "", model = "", temperature = 0.0, use_llm = FALSE), error = function(e) rv$aqua)
          rv$personas <- update_final_scores_from_turns(rv$personas, rv$turns)
          rv$drift_table <- compute_drift_table(rv$personas, rv$turns, rv$model_baseline)
      ...
      if (step_id == 9) {
        withProgress(message = "Options round...", value = 0, {
          ...
          rv$group_options <- res$groups; rv$turns <- res$turns; rv$aqua <- tryCatch(compute_aqua_llm(rv$turns, api_key = "", model = "", temperature = 0.0, use_llm = FALSE), error = function(e) rv$aqua)
          rv$personas <- update_final_scores_from_turns(rv$personas, rv$turns)
          rv$drift_table <- compute_drift_table(rv$personas, rv$turns, rv$model_baseline)
      ...
      if (step_id == 12) {
        withProgress(message = "Evaluation round...", value = 0, {
          ...
          rv$evaluations <- res$groups; rv$turns <- res$turns; rv$aqua <- tryCatch(compute_aqua_llm(rv$turns, api_key = "", model = "", temperature = 0.0, use_llm = FALSE), error = function(e) rv$aqua)
          rv$personas <- update_final_scores_from_turns(rv$personas, rv$turns)
          rv$drift_table <- compute_drift_table(rv$personas, rv$turns, rv$model_baseline)
      ...
      if (step_id == 14 && identical(experiment_mode, "direct_agents")) {
        req(rv$personas, rv$turns)
        # Robust direct report generation: do not restart or overwrite the direct path here.
        # Generate and save the report first; then run post-DRI/post-surveys in protected blocks.
        tryCatch({
          ensure_dri_pre_responses(rv$personas)
        }, error = function(e) {
          rv$status <- paste0("Direct report continuing; DRI pre check failed: ", conditionMessage(e))
        })
        withProgress(message = "Generating direct-condition report...", value = 0, {
          output_type <- input$output_type %||% "policy_recommendation"
          existing_policy <- input$existing_policy %||% ""
          direct_summary_txt <- tryCatch(scalar_text(rv$direct_deliberation$summary %||% ""), error = function(e) "")
          transcript_txt <- scalar_text(rv$transcript %||% "")
          if (nchar(transcript_txt) > 18000) transcript_txt <- paste0(substr(transcript_txt, 1, 18000), "\n\n[Transcript truncated for report generation; full transcript remains stored in the app.]")
          direct_inputs <- paste0(
            "DIRECT DELIBERATION SUMMARY:\n", direct_summary_txt, "\n\n",
            "TRANSCRIPT:\n", transcript_txt, "\n\n",
            "FINAL PERSONA SCORES:\n", safe_json(rv$personas)
          )
          report_instruction <- switch(output_type,
                                       "policy_recommendation" = "Write a policy recommendation report from a direct multi-agent deliberation. Include the problem, the preferred direction, key reasons for and against, implementation conditions, participant initial/final positions, and a short note that this was the direct deliberation condition.",
                                       "policy_support_opposition" = paste0("Write a policy assessment report from a direct multi-agent deliberation evaluating this existing policy: ", scalar_text(existing_policy), ". Include the verdict, reasons for and against, amendments, dissenting positions, participant initial/final positions, and a short note that this was the direct deliberation condition."),
                                       "idea_generation" = "Write an idea-generation report from a direct multi-agent deliberation. Include the challenge, idea catalogue, innovative ideas, consensus ideas, contested ideas, feasibility reflections, next steps, participant initial/final positions, and a short note that this was the direct deliberation condition.")
          incProgress(0.12, detail = "Writing direct report...")
          rv$final_report <- tryCatch({
            call_openrouter(
              input$api_key, input$model,
              list(
                list(role = "system", content = "You write a final deliberation report."),
                list(role = "user", content = synthesis_prompt("Write the final deliberation report", issue, question, evidence_block, direct_inputs, report_instruction))
              ),
              max_tokens = 2800, temperature = 0.25
            )
          }, error = function(e) {
            paste0("DIRECT DELIBERATION REPORT GENERATION FAILED\n\n", conditionMessage(e), "\n\nDirect summary:\n", direct_summary_txt)
          })
          incProgress(0.18, detail = "Writing direct appendix...")
          appendix_instruction <- "Write a detailed appendix for the direct deliberation condition. Include the process description, main argumentative blocs, strongest reasons and counter-reasons, consensus points, unresolved disagreements, observed shifts in participant positions, minority views, evidence gaps, and implications of using a direct condition instead of the staged pipeline."
          rv$appendix_report <- tryCatch({
            call_openrouter(
              input$api_key, input$model,
              list(
                list(role = "system", content = "You write detailed analytical appendices for deliberation reports."),
                list(role = "user", content = synthesis_prompt("Write the appendix for this deliberation", issue, question, evidence_block, direct_inputs, appendix_instruction))
              ),
              max_tokens = 2400, temperature = 0.3
            )
          }, error = function(e) {
            paste0("DIRECT APPENDIX GENERATION FAILED\n\n", conditionMessage(e))
          })
          rv$appendix_report <- paste0(build_transparency_disclosure(rv, input, condition_label = "direct_agents"), rv$appendix_report)
          rv$personas <- update_final_scores_from_turns(rv$personas, rv$turns)
          if (!"final_score" %in% names(rv$personas) || all(!is.finite(rv$personas$final_score))) rv$personas$final_score <- rv$personas$initial_score
          add_replog("appendix")
          save_condition_snapshot("direct_agents")
          incProgress(0.15, detail = "Direct report saved. Running post DRI...")
          
          if (!is.null(rv$auto_dri_survey)) {
            cards <- lapply(seq_len(nrow(rv$personas)), function(i) as.list(rv$personas[i, ]))
            auto_post_results <- vector("list", length(cards))
            for (i in seq_along(cards)) {
              incProgress(0.25 / max(1, length(cards)), detail = paste0("Auto DRI post: agent ", i, "/", length(cards)))
          ...
          if (!is.null(rv$user_dri_survey)) {
            ...
            for (i in seq_along(cards)) {
              incProgress(0.15 / max(1, length(cards)), detail = paste0("User DRI post: agent ", i, "/", length(cards)))
            ...
            add_replog("user_dri_post")
            save_condition_snapshot("direct_agents")
          }
          
          if (isTRUE(rv$survey_enabled) && nzchar(rv$survey_text %||% "")) {
            cards <- lapply(seq_len(nrow(rv$personas)), function(i) as.list(rv$personas[i, ]))
            post_results <- vector("list", length(cards))
            for (i in seq_along(cards)) {
              incProgress(0.10 / max(1, length(cards)), detail = paste0("Optional post survey: agent ", i, "/", length(cards)))
          ...
          incProgress(0.05, detail = "Direct report pipeline complete.")
        })
        rv$step <- 15
        rv$status <- "Direct-condition final report and post-deliberation metrics created."
        add_replog("final_recommendation_direct")
        save_condition_snapshot("direct_agents")
      }
      ...
      if (step_id == 14 && !identical(experiment_mode, "direct_agents")) {
        withProgress(message = "Final recommendation...", value = 0.1, {
          ...
          # Prepend transparency disclosure block (Rountree & Gastil 2026, p. 9)
          rv$appendix_report <- paste0(
            build_transparency_disclosure(rv, input, condition_label = "structured_pipeline"),
            rv$appendix_report)
          add_replog("appendix")
      ...
      if (step_id == 15) {
        withProgress(message = "Quality assessment + AQuA scoring...", value = 0.1, {
          ...
          # Use auditor model for cross-model scrutiny (Rountree & Gastil 2026, p. 8).
          # AQuA scoring remains on the main model because it follows Behrendt et al. (2024)
          # which uses a single scoring model per comment.
          auditor <- input$auditor_model %||% input$model
          out <- call_openrouter(input$api_key, auditor,
      ...
      rv$error <- ""
      if ((rv$current_condition %||% "pipeline") %in% c("pipeline", "direct_agents")) save_condition_snapshot(rv$current_condition)
    }, error = function(e) { rv$error <- conditionMessage(e) })
    rv$loading <- FALSE
  }
  ...
  reset_run_state_for_batch <- function() {
    ...
    rv$central_tendency <- NULL; rv$aqua <- NULL; rv$aqua_strict_failed_reason <- NULL
    # IMPORTANT: also clear the direct-deliberation marker and its companion fields.
    # If rv$direct_deliberation is left non-NULL, run_workflow_step(5) short-circuits
    # at the safety guard ("Direct deliberation already completed") and returns without
    # producing new turns. That left rv$turns empty for runs 2..N of the batch and
    # caused AQuA to be computed only for the first replication. Clearing these
    # fields guarantees every batch iteration produces a fresh transcript/turns,
    # so AQuA (and all turn-derived metrics) are recomputed for every run.
    rv$direct_deliberation <- NULL
    rv$direct_final_report <- NULL
    rv$direct_appendix_report <- NULL
    rv$direct_transcript <- NULL
  ...
  collect_group_batch_row <- function(run_id, condition_label = "direct_deliberation") {
    ...
    aqua_n_llm <- NA_real_
    aqua_n_fallback <- NA_real_
    aqua_method <- NA_character_
    if (!is.null(rv$aqua)) {
      ...
      aqua_n_llm <- suppressWarnings(as.numeric(rv$aqua$overview$n_llm_scored %||% NA))
      aqua_n_fallback <- suppressWarnings(as.numeric(rv$aqua$overview$n_fallback_scored %||% NA))
      aqua_method <- scalar_text(rv$aqua$method_note %||% NA_character_)
    ...
    # One-glance classification of how the AQuA score was produced for this run.
    # Useful as a covariate / filter when analysing the batch dataset:
    #   "pure_llm"             -> every comment was scored by the LLM (n_llm == n_comments, n_fallback == 0)
    #   "mixed_llm_heuristic"  -> some comments LLM-scored, some fell back to the heuristic
    #   "pure_heuristic"       -> no comment was LLM-scored (no API key/model, or LLM disabled)
    #   "failed_strict_llm"    -> AQuA was rejected by strict mode (at least one comment couldn't be LLM-scored).
    #                             The deliberation data IS preserved; only the AQuA columns are NA for this run.
    #   "none"                 -> AQuA could not be computed (no transcript / no scorer succeeded)
    aqua_method_summary <- if (!is.null(rv$aqua_strict_failed_reason) && is.null(rv$aqua)) {
      "failed_strict_llm"
    } else if (is.na(aqua_n_comments) || aqua_n_comments == 0) {
      "none"
    } else if (!is.na(aqua_n_llm) && aqua_n_llm == aqua_n_comments && (is.na(aqua_n_fallback) || aqua_n_fallback == 0)) {
      "pure_llm"
    } else if (!is.na(aqua_n_llm) && aqua_n_llm == 0) {
      "pure_heuristic"
    } else {
      "mixed_llm_heuristic"
    }
    ...
    data.frame(
      ...
      aqua_n_llm_scored = aqua_n_llm, aqua_n_fallback_scored = aqua_n_fallback,
      aqua_method_note = aqua_method, aqua_method_summary = aqua_method_summary,
  ...
  run_batch_post_metrics_no_report <- function(condition_label = "batch") {
    ...
    # =========================================================================
    # Batch AQuA scoring  (strict-mode aware, NON-DESTRUCTIVE)
    # =========================================================================
    # IMPORTANT DESIGN CHOICE: a batch run may take many minutes and many API
    # calls of deliberation BEFORE AQuA scoring even begins. We must NEVER
    # throw away that work just because AQuA had a hiccup. So:
    #
    #   - The run is ALWAYS saved (individual + group rows always written).
    #   - "Strict mode" only affects the AQuA columns:
    #       * If every comment was LLM-scored      -> aqua_score is kept
    #       * If any comment fell back to heuristic -> aqua_score is set to NA
    #         and aqua_method_summary becomes "failed_strict_llm"
    #   - The reason the AQuA was rejected is logged (per-run) in batch_errors,
    #     but the run itself is NOT removed from the group CSV.
    #
    # This guarantees: you never lose deliberation data you already paid for,
    # AND you can still trivially filter with
    #     df[df$aqua_method_summary == "pure_llm", ]
    # to keep only fully-LLM-scored AQuA rows for analysis.
    # =========================================================================
    rv$aqua_strict_failed_reason <- NULL  # cleared each run; set if AQuA was rejected
    
    turns_for_aqua <- tryCatch(get_turns(), error = function(e) NULL)
    strict_aqua <- isTRUE(input$batch_aqua_strict)
    
    if (is.null(turns_for_aqua) || !is.data.frame(turns_for_aqua) || nrow(turns_for_aqua) == 0 || !("text" %in% names(turns_for_aqua))) {
      rv$aqua <- NULL
      rv$aqua_strict_failed_reason <- "no turn-level transcript was produced for this run"
    } else {
      api_key_for_aqua <- input$api_key %||% ""
      model_for_aqua   <- input$model %||% ""
      use_llm_for_aqua <- nzchar(clean_api_key(api_key_for_aqua)) && nzchar(model_for_aqua)
      
      # Always attempt AQuA scoring with whatever's available.
      rv$aqua <- tryCatch(
        compute_aqua_llm(turns_for_aqua,
                         api_key = api_key_for_aqua,
                         model = model_for_aqua,
                         temperature = 0.0,
                         use_llm = use_llm_for_aqua),
        error = function(e) {
          rv$aqua_strict_failed_reason <<- paste0("AQuA scoring pass failed: ", conditionMessage(e))
          NULL
        }
      )
      
      if (is.null(rv$aqua)) {
        if (is.null(rv$aqua_strict_failed_reason)) {
          rv$aqua_strict_failed_reason <- "AQuA scorer returned no result (possible network/auth error)"
        }
      } else {
        # In strict mode, reject AQuA if ANY comment fell back or LLM wasn't used at all.
        if (strict_aqua) {
          n_total    <- suppressWarnings(as.numeric(rv$aqua$overview$n_comments       %||% NA))
          n_llm      <- suppressWarnings(as.numeric(rv$aqua$overview$n_llm_scored     %||% NA))
          n_fallback <- suppressWarnings(as.numeric(rv$aqua$overview$n_fallback_scored %||% NA))
          
          if (!use_llm_for_aqua) {
            rv$aqua_strict_failed_reason <- "no API key or model configured (strict mode requires LLM scoring)"
            rv$aqua <- NULL
          } else if (is.na(n_total) || n_total == 0) {
            rv$aqua_strict_failed_reason <- "zero comments were scored"
            rv$aqua <- NULL
          } else if (is.na(n_llm) || n_llm < n_total || (!is.na(n_fallback) && n_fallback > 0)) {
            n_bad <- if (!is.null(rv$aqua$comments) && is.data.frame(rv$aqua$comments) &&
                         "aqua_scoring_method" %in% names(rv$aqua$comments)) {
              sum(rv$aqua$comments$aqua_scoring_method != "llm", na.rm = TRUE)
            } else {
              if (!is.na(n_fallback)) as.integer(n_fallback) else NA_integer_
            }
            # Pull the most common per-comment failure reason from compute_aqua_llm()
            # so the user can actually diagnose why scoring failed.
            failure_diag <- rv$aqua$failure_summary %||% NA_character_
            rv$aqua_strict_failed_reason <- sprintf(
              "%s of %s comments could not be LLM-scored (strict mode rejects this run's AQuA). Most common failure: %s",
              if (is.na(n_bad)) "?" else as.character(as.integer(n_bad)),
              as.character(as.integer(n_total)),
              if (is.na(failure_diag) || !nzchar(failure_diag)) "unknown (no per-comment error captured)" else failure_diag
            )
            rv$aqua <- NULL
          }
        }
        ...
        if (!is.null(rv$aqua)) {
          add_replog(if (use_llm_for_aqua && strict_aqua) "aqua_llm_scoring_batch_strict_clean"
                     else if (use_llm_for_aqua)          "aqua_llm_scoring_batch_no_report"
                     else                                  "aqua_heuristic_scoring_batch_no_report")
        }
      }
    }
    
    # If AQuA was rejected, record the reason but DO NOT abort the run.
    # collect_group_batch_row() will see rv$aqua == NULL and write NA AQuA columns
    # with aqua_method_summary == "failed_strict_llm" (or "none" if no transcript).
    if (!is.null(rv$aqua_strict_failed_reason)) {
      try({
        err_row <- data.frame(
          run_id = NA_integer_, condition = "direct_deliberation_aqua_only",
          model = input$model %||% "",
          error = paste0("[AQuA-only failure, run preserved] ", rv$aqua_strict_failed_reason),
          stringsAsFactors = FALSE
        )
  ...
  observeEvent(input$run_batch, {
    ...
    steps <- c(1, 5)  # dataset-only batch skips report/appendix and full quality report generation
    ...
    withProgress(message = paste0("Running ", n_runs, " concrete replication(s)..."), value = 0, {
      for (rr in seq_len(n_runs)) {
        ...
        run_result <- tryCatch({
          ...
          run_batch_post_metrics_no_report("direct_deliberation")
          
          ind_rows <- collect_individual_batch_rows(run_id, "direct_deliberation")
          grp_rows <- collect_group_batch_row(run_id, "direct_deliberation")
        ...
        if (!is.null(run_result)) {
          ...
          err_row <- data.frame(run_id = run_id, condition = "direct_deliberation", model = input$model %||% "", error = run_error %||% "unknown error", stringsAsFactors = FALSE)
          checkpoint_batch("errors", err_row)
  ...
  output$dl_batch_individual <- downloadHandler(filename = function() paste0("direct_deliberation_individual_level_batch.csv"), content = function(file) { restore_batch_checkpoints(); utils::write.csv(safe_batch_df(rv$batch_individual), file, row.names = FALSE, na = "") })
  output$dl_batch_group <- downloadHandler(filename = function() paste0("direct_deliberation_group_level_batch.csv"), content = function(file) { restore_batch_checkpoints(); utils::write.csv(safe_batch_df(rv$batch_group), file, row.names = FALSE, na = "") })
  
  output$dl_batch_errors <- downloadHandler(filename = function() paste0("direct_deliberation_failed_runs.csv"), content = function(file) { restore_batch_checkpoints(); utils::write.csv(safe_batch_df(rv$batch_errors %||% data.frame()), file, row.names = FALSE, na = "") })
